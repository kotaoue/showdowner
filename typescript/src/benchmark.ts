...
function benchmarkPrimeNumbers(): BenchmarkResult {
    ...
    return {
        ...
        memory_bytes: 0,  // TypeScriptでは正確なメモリ測定が困難
        operations: count,
...
function benchmarkMatrixMultiplication(): BenchmarkResult {
    const start = process.hrtime.bigint();
    
    const size = 500;
    const a: number[][] = Array(size).fill(null).map(() => Array(size).fill(null).map(() => Math.random()));
    const b: number[][] = Array(size).fill(null).map(() => Array(size).fill(null).map(() => Math.random()));
    const c: number[][] = Array(size).fill(null).map(() => Array(size).fill(0));
...
function benchmarkCryptographicHashing(): BenchmarkResult {
    const start = process.hrtime.bigint();
...
function benchmarkMathOperations(): BenchmarkResult {
    const start = process.hrtime.bigint();
...
function benchmarkLargeArraySort(): BenchmarkResult {
    const start = process.hrtime.bigint();
    
    const size = 1000000;
    const data: number[] = Array(size).fill(null).map(() => Math.floor(Math.random() * 1000001));
...
function benchmarkMemoryAllocation(): BenchmarkResult {
    const start = process.hrtime.bigint();
    ...
    for (let i = 0; i < allocations; i++) {
        const sliceData: number[] = Array(256).fill(i % 256); // 1KB相当のデータ
...
function benchmarkStringConcatenation(): BenchmarkResult {
    const start = process.hrtime.bigint();
